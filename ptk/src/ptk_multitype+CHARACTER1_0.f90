...
#ifdef __PTK_MPI
  call MPI_Bcast(buffer,len(buffer),__PTK_MPI_CHARACTER1, &
...
#ifdef __PTK_MPI
  call MPI_Send(buffer,len(buffer),__PTK_MPI_CHARACTER1, &
...
subroutine ptk_recv_CHARACTER1_0(buffer,source,tag,comm,status,ierr)
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Recv(buffer,len(buffer),__PTK_MPI_CHARACTER1, &
...
subroutine ptk_reduce_CHARACTER1_0(sendbuf,recvbuf,op,root,comm,ierr)
  ...
  comm_loc = ptk_comm_world
  if(present(comm)) comm_loc = comm
  if(len(sendbuf) /= len(recvbuf)) ierr_loc = __LINE__
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Reduce(sendbuf,recvbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
...
subroutine ptk_allreduce_CHARACTER1_0(sendbuf,recvbuf,op,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allreduce(sendbuf,recvbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
...
# 282 "ptk_multitype.spp"
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Reduce(buffer,tempbuf,len(buffer),__PTK_MPI_CHARACTER1, &
                  op%op,root_loc,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
...
# 320 "ptk_multitype.spp"
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Allreduce(buffer,tempbuf,len(buffer),__PTK_MPI_CHARACTER1, &
                     op%op,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
  buffer = tempbuf
#endif
...
subroutine ptk_gather_CHARACTER1_0(sendbuf,recvbuf,root,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
  if(len(sendbuf) * size_loc /= len(recvbuf)*size(recvbuf)) ierr_loc = __LINE__
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Gather(sendbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
                  recvbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
                  root_loc,comm_loc%comm,ierr_loc)
#else
...
subroutine ptk_allgather_CHARACTER1_0(sendbuf,recvbuf,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allgather(sendbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
                     recvbuf,len(sendbuf),__PTK_MPI_CHARACTER1, &
...
subroutine ptk_dummy_CHARACTER1_0
write(0,*)
end subroutine ptk_dummy_CHARACTER1_0
...
# 277 "ptk_multitype.spp"
  CHARACTER (kind=__PTK_CHARACTER1,len=len(buffer)) &
     :: tempbuf (lbound(buffer,1):ubound(buffer,1))
# 282 "ptk_multitype.spp"
  ierr_loc = 0
  root_loc = 0
  if(present(root)) root_loc = root
...
subroutine ptk_dummy_CHARACTER1_1
write(0,*)
end subroutine ptk_dummy_CHARACTER1_1
...
subroutine ptk_dummy_CHARACTER1_2
write(0,*)
end subroutine ptk_dummy_CHARACTER1_2
