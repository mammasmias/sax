...
#ifdef __PTK_MPI
  call MPI_Bcast(buffer,1,__PTK_MPI_LOGICAL3, &
...
#ifdef __PTK_MPI
  call MPI_Send(buffer,1,__PTK_MPI_LOGICAL3, &
...
subroutine ptk_recv_LOGICAL3_0(buffer,source,tag,comm,status,ierr)
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Recv(buffer,1,__PTK_MPI_LOGICAL3, &
...
subroutine ptk_get_count_LOGICAL3_0(buffer,status,count,ierr)
  ...
  integer :: ierr_loc
#ifdef __PTK_MPI
  call MPI_Get_count(status%status,__PTK_MPI_LOGICAL3,count,ierr_loc)
#else
  count = 0
  ierr_loc = __LINE__
#endif
...
#ifdef __PTK_MPI
  call MPI_Reduce(sendbuf,recvbuf,1,__PTK_MPI_LOGICAL3, &
...
subroutine ptk_allreduce_LOGICAL3_0(sendbuf,recvbuf,op,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allreduce(sendbuf,recvbuf,1,__PTK_MPI_LOGICAL3, &
...
# 280 "ptk_multitype.spp"
  LOGICAL (kind=__PTK_LOGICAL3) :: tempbuf 
# 282 "ptk_multitype.spp"
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Reduce(buffer,tempbuf,1,__PTK_MPI_LOGICAL3, &
                  op%op,root_loc,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
...
# 320 "ptk_multitype.spp"
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Allreduce(buffer,tempbuf,1,__PTK_MPI_LOGICAL3, &
                     op%op,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
  buffer = tempbuf
#endif
...
subroutine ptk_gather_LOGICAL3_0(sendbuf,recvbuf,root,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Gather(sendbuf,1,__PTK_MPI_LOGICAL3, &
                  recvbuf,1,__PTK_MPI_LOGICAL3, &
                  root_loc,comm_loc%comm,ierr_loc)
#else
...
subroutine ptk_allgather_LOGICAL3_0(sendbuf,recvbuf,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allgather(sendbuf,1,__PTK_MPI_LOGICAL3, &
                     recvbuf,1,__PTK_MPI_LOGICAL3, &
...
subroutine ptk_dummy_LOGICAL3_0
write(0,*)
end subroutine ptk_dummy_LOGICAL3_0
...
subroutine ptk_bcast_LOGICAL3_1(buffer,root,comm,ierr)
  ...
  if(present(root)) root_loc = root
#ifdef __PTK_MPI
  call MPI_Bcast(buffer,size(buffer),__PTK_MPI_LOGICAL3, &
                 root_loc,comm_loc%comm,ierr_loc)
#endif
  if(present(ierr)) then
    ierr = ierr_loc
...
subroutine ptk_send_LOGICAL3_1(buffer,dest,tag,comm,ierr)
  ...
  if(present(tag)) tag_loc = tag
#ifdef __PTK_MPI
  call MPI_Send(buffer,size(buffer),__PTK_MPI_LOGICAL3, &
                dest,tag_loc,comm_loc%comm,ierr_loc)
#else
  ierr_loc = __LINE__
#endif
...
subroutine ptk_recv_LOGICAL3_1(buffer,source,tag,comm,status,ierr)
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Recv(buffer,size(buffer),__PTK_MPI_LOGICAL3, &
                source_loc,tag_loc,comm_loc%comm,status_loc%status,ierr_loc)
#else
# 162 "ptk_multitype.spp"
  buffer = .true.
...
subroutine ptk_reduce_LOGICAL3_1(sendbuf,recvbuf,op,root,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Reduce(sendbuf,recvbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                  op%op,root_loc,comm_loc%comm,ierr_loc)
#else
  recvbuf = sendbuf
#endif
...
subroutine ptk_allreduce_LOGICAL3_1(sendbuf,recvbuf,op,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allreduce(sendbuf,recvbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                     op%op,comm_loc%comm,ierr_loc)
#else
  recvbuf = sendbuf
#endif
...
subroutine ptk_reduce_ip_LOGICAL3_1(buffer,op,root,comm,ierr)
  ...
  type(ptk_comm) :: comm_loc
# 280 "ptk_multitype.spp"
  LOGICAL (kind=__PTK_LOGICAL3) :: tempbuf (lbound(buffer,1):ubound(buffer,1))
# 282 "ptk_multitype.spp"
  ierr_loc = 0
  root_loc = 0
  if(present(root)) root_loc = root
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Reduce(buffer,tempbuf,size(buffer),__PTK_MPI_LOGICAL3, &
                  op%op,root_loc,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
  call ptk_comm_rank(comm_loc,rank,ierr_loc)
  if(ierr_loc/=0) goto 1
...
# 320 "ptk_multitype.spp"
  ...
  if(present(comm)) comm_loc = comm
#ifdef __PTK_MPI
  call MPI_Allreduce(buffer,tempbuf,size(buffer),__PTK_MPI_LOGICAL3, &
                     op%op,comm_loc%comm,ierr_loc)
  if(ierr_loc/=0) goto 1
  buffer = tempbuf
#endif
...
subroutine ptk_gather_LOGICAL3_1(sendbuf,recvbuf,root,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Gather(sendbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                  recvbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                  root_loc,comm_loc%comm,ierr_loc)
#else
...
subroutine ptk_allgather_LOGICAL3_1(sendbuf,recvbuf,comm,ierr)
  ...
  if(ierr_loc/=0) goto 1
#ifdef __PTK_MPI
  call MPI_Allgather(sendbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                     recvbuf,size(sendbuf),__PTK_MPI_LOGICAL3, &
                     comm_loc%comm,ierr_loc)
#else
...
subroutine ptk_dummy_LOGICAL3_1
write(0,*)
end subroutine ptk_dummy_LOGICAL3_1
...
subroutine ptk_reduce_ip_LOGICAL3_2(buffer,op,root,comm,ierr)
  ...
  type(ptk_comm) :: comm_loc
# 280 "ptk_multitype.spp"
  LOGICAL (kind=__PTK_LOGICAL3) :: tempbuf (lbound(buffer,1):ubound(buffer,1),lbound(buffer,2):ubound(buffer,2))
# 282 "ptk_multitype.spp"
  ierr_loc = 0
  root_loc = 0
  if(present(root)) root_loc = root
...
subroutine ptk_dummy_LOGICAL3_2
write(0,*)
end subroutine ptk_dummy_LOGICAL3_2
