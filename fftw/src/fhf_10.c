...
/* Generated on Mon Mar 24 02:08:07 EST 2003 */

#include "fftw-int.h"
#include "fftw.h"

/* Generated by: /homee/stevenj/cvs/fftw/gensrc/genfft -magic-alignment-check -magic-twiddle-load-all -magic-variables 4 -magic-loopi -hc2hc-forward 10 */
...
/*
 * Generator Id's : 
 * $Id: fhf_10.c,v 1.1.1.1 2004/12/28 16:08:07 software Exp $
 * $Id: fhf_10.c,v 1.1.1.1 2004/12/28 16:08:07 software Exp $
 * $Id: fhf_10.c,v 1.1.1.1 2004/12/28 16:08:07 software Exp $
 */

void fftw_hc2hc_forward_10(fftw_real *A, const fftw_complex *W,
			   int iostride, int m, int dist)
{
     ...
     Y = A + (10 * iostride);
     {
	  ...
	  tmp181 = tmp176 + tmp177;
	  {
	       ...
	       tmp167 = tmp160 + tmp161;
	       tmp163 = X[6 * iostride];
	       tmp164 = X[iostride];
	       tmp165 = tmp163 - tmp164;
	       tmp168 = tmp163 + tmp164;
	  }
	  tmp174 = tmp162 + tmp165;
	  tmp183 = tmp167 + tmp168;
	  {
	       fftw_real tmp153;
	       fftw_real tmp154;
	       fftw_real tmp156;
	       ...
	       tmp170 = tmp153 + tmp154;
	       tmp156 = X[8 * iostride];
	       tmp157 = X[3 * iostride];
	       tmp158 = tmp156 - tmp157;
	       tmp171 = tmp156 + tmp157;
	  }
	  ...
	  tmp182 = tmp170 + tmp171;
	  {
	       ...
	       Y[-iostride] =
		   -((K951056516 * tmp159) + (K587785252 * tmp166));
	       Y[-3 * iostride] =
		   (K587785252 * tmp159) - (K951056516 * tmp166);
	  ...
	  }
	  {
	       ...
	       Y[-2 * iostride] =
		   (K951056516 * tmp169) - (K587785252 * tmp172);
	       Y[-4 * iostride] =
		   (K951056516 * tmp172) + (K587785252 * tmp169);
	       tmp175 = K559016994 * (tmp173 - tmp174);
	       tmp179 = tmp173 + tmp174;
	       tmp180 = tmp178 - (K250000000 * tmp179);
	       X[iostride] = tmp175 + tmp180;
	       X[3 * iostride] = tmp180 - tmp175;
	       X[5 * iostride] = tmp179 + tmp178;
	  }
     }
     X = X + dist;
     Y = Y - dist;
