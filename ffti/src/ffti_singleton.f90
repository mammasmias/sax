...
module ffti_singleton_module
implicit none

contains

subroutine ffti_singleton3d(f,n1,n2,n3,sgn)
implicit none
complex, intent(inout) :: f(:)
integer, intent(in)    :: n1,n2,n3,sgn
integer :: isn,ntot,n,nspan
real :: a(size(f)),b(size(f))
if(size(f)/=n1*n2*n3) stop "ERROR IN ffti_singleton3d"
a = real(f)
b = aimag(f)
isn = sgn
ntot = n1*n2*n3
nspan= n1
n    = n1
call ffti_singleton(a,b,ntot,n,nspan,isn)
isn = sgn
ntot = n1*n2*n3
nspan= n1*n2
n    = n2
...
call ffti_singleton(a,b,ntot,n,nspan,isn)
f=cmplx(a,b)
end subroutine ffti_singleton3d

end module ffti_singleton_module



      subroutine ffti_singleton(a,b,ntot,n,nspan,isn)
...
!  error finish, insufficient array storage
  998 isn=0
      write(*,*) "array bounds exceeded within subroutine fft"
      stop
      end
