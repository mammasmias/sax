...
#include "fftw.h"
#include "ffti_cdefs.h"


int FC_FUNC_(ffti_create_plan_1d,FFTI_CREATE_PLAN_1D)(fftw_plan *p, int *n, int *idir)
{
...
int FC_FUNC_(ffti_destroy_plan_1d,FFTI_DESTROY_PLAN_1D)(fftw_plan *p)
{
...
int FC_FUNC_(ffti_create_plan_2d,FFTI_CREATE_PLAN_2D)(fftwnd_plan *p, int *n, int *m, int *idir)
{
   fftw_direction dir = ( (*idir < 0) ? FFTW_FORWARD : FFTW_BACKWARD );
...
int FC_FUNC_(ffti_destroy_plan_2d,FFTI_DESTROY_PLAN_2D)(fftwnd_plan *p)
{
...
int FC_FUNC_(ffti_create_plan_3d,FFTI_CREATE_PLAN_3D)(fftwnd_plan *p, int *n, int *m, int *l, int *idir)
{
   fftw_direction dir = ( (*idir < 0) ? FFTW_FORWARD : FFTW_BACKWARD );
...
int FC_FUNC_(ffti_destroy_plan_3d,FFTI_DESTROY_PLAN_3D)(fftwnd_plan *p)
{
   if ( *p != NULL ) fftwnd_destroy_plan(*p);
...
int FC_FUNC_(ffti_x_stick,FFTI_X_STICK)
...
int FC_FUNC_(ffti_y_stick,FFTI_Y_STICK) (fftw_plan *p, FFTW_COMPLEX *a, int *ny, int *ldx )
{
...
int FC_FUNC_(ffti_z_stick,FFTI_Z_STICK) (fftw_plan *p, FFTW_COMPLEX *zstick, int *ldz, int *nstick_l)
{
   int howmany, idist;
   howmany = (*nstick_l) ;
   idist   = (*ldz);
...
int FC_FUNC_(Ffti_inplace_drv_1d,FFTI_INPLACE_DRV_1D)(fftw_plan *p, int *nfft, FFTW_COMPLEX *a, int *inca, int *idist)
{
   fftw(*p, (*nfft), a, (*inca), (*idist), 0, 0, 0);
   return 0;
}

int FC_FUNC_(ffti_inplace_drv_2d,FFTI_INPLACE_DRV_2D)( fftwnd_plan *p, int *nfft, FFTW_COMPLEX *a, int *inca, int *idist)
{
   fftwnd( *p, (*nfft), a, (*inca), (*idist), 0, 0, 0 );
   return 0;
}

int FC_FUNC_(ffti_inplace_drv_3d,FFTI_INPLACE_DRV_3D)( fftwnd_plan *p, int *nfft, FFTW_COMPLEX *a, int *inca, int *idist)
{
   fftwnd( *p, (*nfft), a, (*inca), (*idist), 0, 0, 0 );
   return 0;
}
